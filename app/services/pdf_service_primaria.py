...
class BoletaPrimariaPDF(FPDF):
    ...
    def _draw_area_cell(self, x, y, w, h, text):
        """Celda de área mergeada verticalmente con texto centrado y auto-wrap."""
    ...
    def _grades_table(self, ctx):
        ...
        # Row 1: PROMEDIO FINAL (PFW + NLW spans HR1)
        x_pf = x_bim + nt * BW
        self.set_xy(x_pf, y0)
        ...
        for area_display, course_names in boleta_layout:
            ...
            nivel = ctx['student'].nivel if ctx.get('student') else 'PRIMARIA'
            x_pf = ML + AW + CW + nt * BW
            if n == 1:
    ...
    def _draw_course_grades_row(self, cid, data, terms, eda_data):
        ...
        El PROMEDIO FINAL se dibuja a nivel de ÁREA en _grades_table."""
        for term in terms:
    ...
    def _behavior_ppff_row(self, ctx):
        ...
        self.multi_cell(
            w_half, 4.5,
            self._safe('EVALUACION DE RESPONSABILIDAD DEL PPFF O APODERADO'),
            border=1, align='C', fill=True)
        self._reset()
        y3 = self.get_y()
