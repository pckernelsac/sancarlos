...
RH   = 4.6     # alto fila (balance legibilidad vs 1 hoja A4)
HR1  = 5.5     # alto fila header bimestre
HR2  = 4.2     # alto fila sub-header P1/P2

# Columnas tabla de notas (P1/P2 algo más estrechos para PF + NIVEL DE LOGRO)
AW   = 17      # área (mergeada)
CW   = 33      # asignatura
P1W  = 7       # EDA 1
P2W  = 7       # EDA 2
...
PFW  = 11      # promedio final (evita desborde numérico)
NLW  = PW - AW - CW - 4 * BW - PFW   # nivel de logro (~13 mm)

# ── Orden exacto de la boleta (fiel a la imagen) ─────────────────────────────
# Cada tupla: (nombre_display_area, [nombres_de_curso_en_BD])
# Si la lista tiene 1 solo curso → fila standalone (area+asignatura fusionadas)
# Si tiene >1 → area mergeada verticalmente + filas de curso

# Layout común (los cursos que no existan para un grado se omiten automáticamente)
BOLETA_LAYOUT_PRIMARIA = {
    # 1° y 2°: sin ÁLGEBRA, sin FÍSICA/QUÍMICA/BIOLOGÍA/METODOLOGÍA, con CIENCIA Y TECNOLOGÍA
    "1": [
        ("COMUNICACIÓN",        ["COMUNICACIÓN", "RAZ. VERBAL", "PLAN LECTOR"]),
        ("MATEMÁTICAS",         ["ARITMÉTICA", "GEOMETRÍA", "ESTADÍSTICA", "RAZ. MATEMÁTICO"]),
    ...
    "4": [
        ("COMUNICACIÓN",        ["COMUNICACIÓN", "RAZ. VERBAL", "PLAN LECTOR"]),
        ("MATEMÁTICAS",         ["ARITMÉTICA", "ÁLGEBRA", "GEOMETRÍA", "ESTADÍSTICA", "RAZ. MATEMÁTICO"]),
        ("PERSONAL SOCIAL",     ["PERSONAL SOCIAL"]),
        ("CIENCIA Y\nTECNOLOGÍA", ["CIENCIA Y TECNOLOGÍA"]),
        ("EDUCACIÓN RELIGIOSA", ["EDUCACIÓN RELIGIOSA"]),
        ("EDUCACIÓN FÍSICA",    ["EDUCACIÓN FÍSICA"]),
    ...
    "5": [
        ...
        ("PERSONAL SOCIAL",             ["PERSONAL SOCIAL"]),
        ("BIOLOGÍA",                    ["BIOLOGÍA"]),
        ("METODOLOGÍA DEL APRENDIZAJE", ["METODOLOGÍA"]),
        ("EDUCACIÓN RELIGIOSA",         ["EDUCACIÓN RELIGIOSA"]),
        ("EDUCACIÓN FÍSICA",            ["EDUCACIÓN FÍSICA"]),
        ("ARTE Y CULTURA",              ["ARTE Y CULTURA"]),
        ("INGLÉS",                      ["INGLÉS"]),
    ...
    "6": [
        ...
        ("MATEMÁTICAS",                 ["ARITMÉTICA", "ÁLGEBRA", "GEOMETRÍA", "ESTADÍSTICA", "RAZ. MATEMÁTICO"]),
        ("PERSONAL SOCIAL",             ["PERSONAL SOCIAL"]),
        ("CIENCIA Y\nTECNOLOGÍA",      ["BIOLOGÍA", "FÍSICA", "QUÍMICA"]),
        ("METODOLOGÍA DEL APRENDIZAJE", ["METODOLOGÍA"]),
        ("EDUCACIÓN RELIGIOSA",         ["EDUCACIÓN RELIGIOSA"]),
        ("EDUCACIÓN FÍSICA",            ["EDUCACIÓN FÍSICA"]),
        ("ARTE Y CULTURA",              ["ARTE Y CULTURA"]),
...
# 2° usa el mismo layout que 1°
BOLETA_LAYOUT_PRIMARIA["2"] = BOLETA_LAYOUT_PRIMARIA["1"]

# Fallback para compatibilidad
BOLETA_LAYOUT = BOLETA_LAYOUT_PRIMARIA["6"]


# ── Clase principal ────────────────────────────────────────────────────────────
class BoletaPrimariaPDF(FPDF):
    ...
    def _draw_area_cell(self, x, y, w, h, text):
        """Celda de área mergeada verticalmente con texto centrado.
        IMPORTANTE: guarda y restaura la posición del cursor."""
        ...
        self._reset()
        self.set_xy(sx, sy)  # restaurar posición
    ...
    def _header(self, anio):
        """Encabezado compacto para dejar espacio al cuerpo (1 hoja A4)."""
    ...
    def _student_info(self, student, anio):
        ...
        grado_txt = grado_map.get(str(student.grado), str(student.grado))

        self.set_font('Helvetica', 'B', 9)
        self._fc(BRAND); self._tc(WHITE)
        self.cell(PW, 4.8,
                  self._safe(f'INFORME ACADEMICO: {grado_txt} DE PRIMARIA'),
    ...
    def _grades_table(self, ctx):
        ...
        # Seleccionar layout según grado del estudiante
        grado = str(ctx['student'].grado) if ctx.get('student') else "6"
        boleta_layout = BOLETA_LAYOUT_PRIMARIA.get(grado, BOLETA_LAYOUT)
        ...
        y_row2 = y0 + HR1                      # inicio sub-headers
        y_data = y0 + HR1 + HR2                # inicio datos

        # Row 1: celdas que ocupan 2 filas (AREAS, ASIG, PF, NL)
        self.set_xy(ML, y0)
        self._hcell(AW, HR1 + HR2, 'AREAS', size=8)
        self._hcell(CW, HR1 + HR2, 'ASIGNATURAS', size=8, align='L')

        # Row 1: celdas bimestre (solo fila 1)
        x_bim = ML + AW + CW
        ...
        # Row 1: PF y NIVEL DE LOGRO (2 filas)
        x_pf = x_bim + nt * BW
        self.set_xy(x_pf, y0)
        self._hcell(PFW, HR1 + HR2, '', size=8)
        self._hcell(NLW, HR1 + HR2, 'NIVEL DE\nLOGRO', size=6.5)

        # Row 2: sub-headers P1/P2/PROM/LETRA dentro de cada bimestre
        ...
        for area_display, course_names in boleta_layout:
            ...
            if n == 1:
                ...
                # Multi-curso: area mergeada verticalmente
                area_h = n * RH
        ...
        self._hcell(AW + CW, RH, 'Orden de Merito', size=8)
        for i, term in enumerate(terms):
            r = merit_pt.get(term.id)
            label = bim_short[i] if i < 4 else ''
            self._hcell(P1W + P2W, RH, label, size=7.5)
            self._hcell(PMW + QW, RH, f'{r}' if r else '-', size=8)
        self._hcell(PFW, RH, 'PF', size=7.5)
        self._hcell(NLW, RH, f'{merit_f}' if merit_f else '-', size=7,
                    nx='LMARGIN', ny='NEXT')
        self.ln(1.5)

    def _draw_course_grades_row(self, cid, data, terms, eda_data):
        """Dibuja columnas de notas de un curso en la fila actual.
        Asume que X ya está posicionado después de la celda de asignatura."""
        for term in terms:
        ...
        # PF / nivel (tipografía ajustada al ancho de columna)
...
# ── Funciones públicas ─────────────────────────────────────────────────────────
def generate_boleta_primaria_pdf(ctx: dict) -> bytes:
    return BoletaPrimariaPDF().build(ctx)


def generate_bulk_boletas_primaria_pdf(ctx_list: list) -> bytes:
    return BoletaPrimariaPDF().build_bulk(ctx_list)
